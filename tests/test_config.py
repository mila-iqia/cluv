...
class TestDataSource:
    def test_remote_data_source_accepted(self, tmp_path: Path) -> None:
        p = write_pyproject(
            tmp_path,
            """
[tool.cluv]
results_path = "logs"
data_source = "mila:/network/datasets/cifar10"
...
""",
        ...
        cfg = load_cluv_config(p)
        assert cfg.data_source == "mila:/network/datasets/cifar10"

    def test_local_data_source_accepted(self, tmp_path: Path) -> None:
        """data_source may be a plain local path with no hostname: prefix."""
        p = write_pyproject(
            tmp_path,
            """
[tool.cluv]
...
""",
        ...
        cfg = load_cluv_config(p)
        assert cfg.data_source is not None
        assert ":" in cfg.data_source
        hostname, _, path = cfg.data_source.partition(":")
        assert hostname == "mila"
        assert path == "/network/datasets/cifar10"

    def test_local_data_source_with_home_tilde(self, tmp_path: Path) -> None:
        """Tilde-prefixed paths are also valid local data_source values."""
        p = write_pyproject(
            tmp_path,
            """
[tool.cluv]
...
""",
        ...
        cfg = load_cluv_config(p)
        assert cfg.data_source == "~/datasets/cifar10"
        assert ":" not in cfg.data_source
