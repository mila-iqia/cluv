...
Adapted from the [mila-docs ImageNet example](https://github.com/mila-iqia/mila-docs/tree/master/docs/examples/advanced/imagenet)
to run on any of the clusters configured in `[tool.cluv]` of the `pyproject.toml`.
...
class Args:
    ...
    wandb_project: str = "cluv-imagenet-example"
...
def main():
    ...
    if args.checkpoint_dir is None:
        # Save checkpoints in the results dir that cluv picked for this run. Cluv also creates a
        # `logs` symlink in the project folder pointing at `results_path`, so the checkpoints of
        # this run show up under `logs/{cluster}_{job_id}/` even though they live on $SCRATCH.
        args.checkpoint_dir = RESULTS_DIR
    ...
    # To view the traces, run `uvx tensorboard --with=torch_tb_profiler --logdir logs`
    profiler = profile(
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        for batch_index, batch in enumerate(
            ...
            with torch.cuda.stream(data_transfer_cuda_stream):
                ...
                x, y = batch
            # The recipe this pattern comes from is explicit that the consuming (default) stream
            # has to wait on the side stream before touching the tensors it produced. This was
            # missing here - reproduced locally as a CUDA assertion inside the loss computation
            # ("t >= 0 && t < n_classes") once real GPU compute ran on the side stream during an
            # experiment with GPU-side transforms; consistent under normal execution and
            # consistently absent under `CUDA_LAUNCH_BLOCKING=1`, a classic race signature. Keeping
            # the wait even though that experiment was reverted: without it, `training_step` below
            # can start reading `x`/`y` on the default stream before the H2D copy above finishes.
            ...
            if (
                ...
                # isn't a multiple of logging interval.
...
def setup_wandb(
    ...
    with goes_first(is_master):
        run = wandb.init(
            ...
            # Keep the wandb files with the rest of this run's results, so that `cluv sync`
            # brings them back along with the checkpoints (useful with WANDB_MODE=offline).
...
def watch_slurm_output(run, policy: str = "live"):
    ...
    """
    run.save(str(RESULTS_DIR / "*.out"), base_path=str(RESULTS_DIR), policy=policy)
...
def make_datasets(
    ...
    All transforms run here, per-sample, on CPU. `RandomResizedCrop`/`Resize` in particular have to:
    real ImageNet images have arbitrary native sizes, so there's no fixed-size tensor to stack into
    a batch until each image has been resized to the common output size individually. Moving the
    remaining, purely-elementwise ops (`ToDtype`/`Normalize`) to run once per batch on the GPU
    instead was tried and worked, but wasn't kept: for this dataset/model, CPU-side transforms
    aren't the bottleneck, so it wasn't worth the extra code path and the CUDA-stream subtleties
    that came with it (see the comment on `data_transfer_cuda_stream` in `main`).
    """
    if use_fake_data:
