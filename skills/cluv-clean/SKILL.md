...
# Cleaning up run results with `cluv clean`

`cluv sync` only ever copies results **from** clusters **to** the local machine — it never
deletes anything on either side. Deleting a run folder locally and re-syncing just re-downloads
it, and every job run adds another folder under `results_path` (usually under `$SCRATCH`) on every
cluster, forever, until something removes it. `cluv clean` is that removal step.

## Usage

```bash
cluv clean --dry-run       # preview what would be deleted, nothing removed
...
Condition 2 exists so a run that finished on the cluster minutes ago (and hasn't been fetched yet)
is never mistaken for one deliberately deleted — `clean` never removes something that hasn't been
seen locally yet. This is also why **a cluster needs at least one prior successful `cluv sync`**
before `clean` will touch it — clusters never synced are skipped, with a note to sync first.
...
- **Per-cluster, independently**: the same job-id folder existing on two clusters is two separate
  ...
  if only one of them is "the" run actually meant to keep.
- **Not recoverable**: deletion is a remote `rm -rf` over SSH — no trash, no undo. Suggest
  `--dry-run` first whenever the user is unsure what will be removed.
...
- **A folder that was deleted locally is still there after `clean`**: check whether it's newer
  than the cluster's last successful sync (condition 2 above) — if so, `clean` is deliberately
  leaving it alone until it's been fetched at least once via `cluv sync`.
