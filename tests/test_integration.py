...
from cluv.cli.submit_utils.vram import get_gpu_request, get_gpu_types
...
async def test_vram_sbatch_args_are_valid(remote: Remote, dont_cache_gpu_types: None):
    """The sbatch flags generated by `--vram` are understood by the real cluster's `sbatch`.

    Uses `sbatch --test-only`, so no job is submitted. A cluster can still refuse to *schedule*
    such a job (Tamia only allocates whole GPU nodes, and Mila's H100s are in their own
    ...
    gpu_types_asked_for = [
        ...
        if (gpu_request := get_gpu_request(sbatch_args)) is not None
    ]
